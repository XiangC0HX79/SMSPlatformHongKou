...
{   //____________________________________________________________________________
    ...
    class SocketListener
    {
        ...
        Dictionary<int, String> emppIDs = new Dictionary<int, string>();
        //Dictionary<String,SMS> smsDict = new Dictionary<string,SMS>();
        ...
        internal void StartListen()
        {
            ...
            try
            {
                Program.testWriter.WriteLine("INDE 我们首次建立连接开始");
                result = this.empp.connect(this.socketListenerSettings.Empp_Host, this.socketListenerSettings.Empp_Port, this.socketListenerSettings.Empp_AccountId, this.socketListenerSettings.Empp_Password);
            }
            catch (Exception ex)
            {
                Program.testWriter.WriteLine("INDE 我们首次建立连接开始，以失败告终");
                Program.testWriter.WriteLine(ex.ToString());
            }
            ...
            {
                Program.testWriter.WriteLine("我们首次连接失败，接下来进行while重连----------" + con);
                con++;
                try
                {
                    result = this.empp.connect(this.socketListenerSettings.Empp_Host, this.socketListenerSettings.Empp_Port, this.socketListenerSettings.Empp_AccountId, this.socketListenerSettings.Empp_Password);
        ...
        private void ProcessReceive(SocketAsyncEventArgs receiveSendEventArgs)
        {
            ...
            if (receiveSendEventArgs.BytesTransferred == 0)
            {
                if (Program.watchProgramFlow == true)   //for testing
                {
                    Program.testWriter.WriteLine("ProcessReceive NO DATA, receiveSendToken id " + receiveSendToken.TokenId);
                }
            ...
            Int32 BytesToProcess = receiveSendEventArgs.BytesTransferred;
            Byte[] bt = new Byte[BytesToProcess];
            Buffer.BlockCopy(receiveSendEventArgs.Buffer, receiveSendEventArgs.Offset, bt, 0, BytesToProcess);

            //收到 <policy-file-request/>\0 后客户端发送策略信息
            if (System.Text.ASCIIEncoding.Default.GetString(bt).Trim() == "<policy-file-request/>\0")
            {
                Program.testWriter.WriteLine(((IPEndPoint)receiveSendEventArgs.AcceptSocket.LocalEndPoint).Address.ToString() + " 接收policy-file-request");
                ...
                receiveSendEventArgs.AcceptSocket.Send(System.Text.ASCIIEncoding.Default.GetBytes(xml));
            ...
            if (incomingTcpMessageIsReady == true)
            {
                ...
                {
                    Program.testWriter.WriteLine(receiveSendToken.TokenId + ", Message in DataHolder = " + Encoding.UTF8.GetString(receiveSendToken.theDataHolder.dataMessageReceived) + "\r\n");
                }
                //for testing only
                ...
                XmlDocument xmlDocReceive = new XmlDocument();
                xmlDocReceive.LoadXml(Encoding.UTF8.GetString(receiveSendToken.theDataHolder.dataMessageReceived));
                ...
                //发送SMS
                XmlNode nodeReceiveMethod = xmlDocReceive.SelectSingleNode("/xml/method");

                //发送激活用户验证码
                if (nodeReceiveMethod.InnerText == "method_sendverification")
                {
                    String phone = xmlDocReceive.SelectSingleNode("/xml/phone").InnerText;

                    String verification = receiveSendToken.xmlDocDataToSend.SelectSingleNode("/xml/verification").InnerText;

                    this.sendSMS(phone, verification,"000");
                }
                //登录时创建SMS
                else if (nodeReceiveMethod.InnerText == "method_login")
                {
                    String phone = xmlDocReceive.SelectSingleNode("/xml/phone").InnerText;

                    //SMS sms = new SMS(receiveSendEventArgs, smsDict, phone);
                }
                else if (nodeReceiveMethod.InnerText == "method_sendSMS")
                {
                    String phone = xmlDocReceive.SelectSingleNode("/xml/phone").InnerText;
                    String message = xmlDocReceive.SelectSingleNode("/xml/message").InnerText;
                    String serviceID = xmlDocReceive.SelectSingleNode("/xml/serviceID").InnerText;

                    //SMS sms = getSMSByReceiveSendEventArgs(receiveSendEventArgs);
                    //if (sms != null)
                    //{
                        String[] aMobs = phone.Split(';');
                        for (int n=0;n<aMobs.Length;n++)
                        {
                            String mob = aMobs[n].Trim();
                            if(mob != "")
                            {
                                this.sendSMS(mob, message, serviceID);
                            }
                        }
                   //}
                }
        ...
        private void SubmitRespInterface(SubmitResp sm)
        {
            ...
            //SMS sms = getSMSBySubmitResp(sm);

            //if (sms != null)
            //{
            //    DataHoldingUserToken receiveSendToken = (DataHoldingUserToken)sms.ReceiveSendEventArgs.UserToken;

            //    StringBuilder sb = new StringBuilder();
            //    sb.Append("<xml>");
            //    sb.Append("<method>response</method>");
            //    sb.Append("<submitDatetime>" + DateTime.Now + "</submitDatetime>");
            //    sb.Append("<message>" + sm.Result + "</message>");
            //    sb.Append("</xml>");

            //    Char[] chArray = new Char[sb.Length];
            //    sb.CopyTo(0, chArray, 0, sb.Length);

            //    DataHolder theDataHolder = new DataHolder();
            //    theDataHolder.dataMessageReceived = Encoding.UTF8.GetBytes(chArray);

            //    receiveSendToken.theMediator.HandleData(theDataHolder);
            //    receiveSendToken.CreateNewDataHolder();
            //    receiveSendToken.Reset();
            //    receiveSendToken.theMediator.PrepareOutgoingData();
        ...
        {
            Program.testWriter.WriteLine("发生了EMPPClose事件了");
        }
        ...
        private void SocketClosed(int errorCode)
        {
            ...
            Program.testWriter.WriteLine("我们现在发生了socketclose事件：" + "errorcoded is :" + errorCode + "我们即将进行重新连接");
            Console.WriteLine(str);
            Reconnect2();
        }
        ...
        private void Reconnect2()
        {
            ...
            Program.testWriter.WriteLine("发生异常，我们正在重新连接");
            ...
            while (result != EMPPLib.ConnectResultEnum.CONNECT_OK && result != EMPPLib.ConnectResultEnum.CONNECT_KICKLAST)
            {
                Program.testWriter.WriteLine("WHILE   发生异常，我们进行重新连接");
            ...
            Program.testWriter.WriteLine("congratulation , now the connection is ok or kicklast");
            Thread.Sleep(3000);
        }
        ...
        private void sendSMS(String pMods, String pMessage, String serviceID)
        {
            ...
            //while (message != "")
            //{
            //    String msg = (message.Length > 60) ? message.Substring(0, 60) : message;
            //    message = (message.Length > 60) ? message.Substring(60, message.Length - 60):"";

                ShortMessage shortMsg = new ShortMessageClass();
                shortMsg.srcID = this.socketListenerSettings.Empp_AccountId;
                shortMsg.ServiceID = serviceID;// this.socketListenerSettings.ser;
                ...
                //Program.testWriter.WriteLine("我们打印原始的短信内容：" + msg);
            ...
            try
            {
                sendID = sc.InsertSend(pMods, pMessage, "SENDING");
            }
            catch (Exception e)
            {
            }

            emppIDs.Add(Convert.ToInt32(empp.SequenceID), sendID);    

                if (empp != null && empp.connected == true)
                {
                    //Program.testWriter.WriteLine("目前的短信的seqid是：" + shortMsg.SequenceID);
                    Program.testWriter.WriteLine("发送短信(sequceid)：" + empp.SequenceID);
                    Console.WriteLine("发送短信(sequceid)：" + empp.SequenceID);

                    Program.testWriter.WriteLine("发送短信(content)：" + shortMsg.content);
                    Console.WriteLine("发送短信(content)：" + shortMsg.content + "(ServiceID)：" + shortMsg.ServiceID);

                    
                    //if (serviceID != "0")
                    //    smsDict[serviceID].smsSendMessages.Add(new SMSSendMessage(shortMsg));

                    empp.submit(shortMsg);

                    //Program.testWriter.WriteLine("IF语句   我们已经发送短信：" + shortMsg.content);
                ...
                {
                    //Program.testWriter.WriteLine("连接已经关闭，我们即将重新连接：现在我们在else语句里面");
                    Reconnect2();
                    //Console.WriteLine("现在的连接状况是: " + empp.connected);

                    //Program.testWriter.WriteLine("\r\n");
                    //Program.testWriter.WriteLine("目前的连接状况是：" + empp.connected);
                    //Program.testWriter.WriteLine("目前的shortmsgseqid是：" + shortMsg.SequenceID);
                    //Program.testWriter.WriteLine("目前的emppseqid是：" + empp.SequenceID);

                    //Program.testWriter.WriteLine("ELSE语句   我们即将发送短信：" + msg);
                    ...
                    //Program.testWriter.WriteLine("ELSE语句   我们已经发送短信：" + msg);
        ...
        //private SMS getSMSByReceiveSendEventArgs(SocketAsyncEventArgs e)
        //{
        //    foreach (KeyValuePair<String, SMS> pair in smsDict)
        //    {
        //        if (pair.Value.ReceiveSendEventArgs == e)
        //        {
        //            return pair.Value;
        //        }
        //    }
        ...
        //private SMS getSMSBySubmitResp(SubmitResp sm)
        //{
        //    foreach (KeyValuePair<String, SMS> pair in smsDict)
        //    {
        //        foreach(SMSSendMessage smsSendMessage in pair.Value.smsSendMessages)
        //        {
        //            if(smsSendMessage.ShortMsg.SequenceID == sm.SequenceID - 1)
        //            {
        //                smsSendMessage.MsgID = sm.MsgID;
